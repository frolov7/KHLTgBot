...
export const selectLeague = async (browser, countryId, sport = "football") => {
  start();
  const leagues = await getListOfLeagues(browser, countryId, sport);
  stop();
  ...
  const { choice } = await inquirer.prompt([
    {
      type: 'list',
      name: 'choice',
      message: `Select a league (${sport}):`, // 👈 добавил отображение спорта
