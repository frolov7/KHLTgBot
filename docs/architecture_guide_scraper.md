...
- Ключевым полем, связывающим эти категории, является id (или match_id).
  Этот идентификатор уникален и совпадает во всех JSON-файлах, чтобы TelegramBOT мог связывать: \* матч → его прогнозы → видеообзор;
...
### 5.3.5 События матчей — /data/events/KHL_EVENTS.json

Модуль scrapeRecentEvents() и вспомогательные функции парсят события матчей КХЛ с сайта Flashscore и сохраняют их в JSON-файл KHL_EVENTS.json.
Эти данные используются для аналитики и последующего отображения в Telegram-боте.

```
{
  "boRXxDHG": {
    "id": "boRXxDHG",
    "home": "SKA St. Petersburg",
    "away": "Novosibirsk",
    "goals": [
      {
        "period": 1,
        ...
        "score": "1-0",
        "scorer": "Короткий М.",
        "assistants": ["Хайруллин М.", "Мерфи Т."],
        "team": "SKA St. Petersburg",
        "goal_type": "Большинство"
      }
    ],
    "penalties": [
      {
    ...
    "disallowed_goals": [
      {
        ...
        "time": "18:10",
        "team": "Novosibirsk",
        "reason": "игра высоко поднятой клюшкой"
      }
    ]
  }
}
...
| Тип события | Ключ в JSON | Пример данных |
| ----------------- | ------------------ | --------------------------- |
| Заброшенные шайбы | `goals` | Автор, ассистенты, тип гола |
| Удаления | `penalties` | Игрок, причина удаления |
| Замены вратарей | `goalie_changes` | Кто ушёл / кто вышел |
| Буллиты | `shootouts` | Исполнитель и результат |
| Отменённые голы | `disallowed_goals` | Причина отмены |

Пример запуска

```
node src/scraper/scraperRunner.js --events
```

После выполнения команда обновляет файл: /data/events/KHL_EVENTS.json

а затем импортирует данные в таблицы:
